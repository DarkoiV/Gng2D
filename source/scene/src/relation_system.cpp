...
RelationSystem::RelationSystem(entt::registry& r)
    : reg(r)
{
    reg.on_construct<Gng2D::Parent>().connect<&RelationSystem::addChildToParent>();
    reg.on_destroy<Gng2D::Parent>().connect<&RelationSystem::removeChildFromParent>();
    reg.on_destroy<Gng2D::Children>().connect<&RelationSystem::destroyAllChildren>();

    reg.on_construct<Gng2D::TransformLayer>()
        .connect<&RelationSystem::setLayerFromTransformLayer>();
    reg.on_update<Gng2D::TransformLayer>().connect<&RelationSystem::setLayerFromTransformLayer>();
    reg.on_update<Gng2D::Layer>().connect<&RelationSystem::updateChildrenLayer>();

    reg.on_construct<TransformPosition>()
        .connect<&RelationSystem::setPositionFromTransformPosition>();
    reg.on_update<TransformPosition>().connect<&RelationSystem::setPositionFromTransformPosition>();
    reg.on_update<Gng2D::Position>().connect<&RelationSystem::updateChildrenPosition>();
...
{
    reg.on_construct<Gng2D::Parent>().disconnect<&RelationSystem::addChildToParent>();
    reg.on_destroy<Gng2D::Parent>().disconnect<&RelationSystem::removeChildFromParent>();
    reg.on_destroy<Gng2D::Children>().disconnect<&RelationSystem::destroyAllChildren>();

    reg.on_construct<Gng2D::TransformLayer>()
        .disconnect<&RelationSystem::setLayerFromTransformLayer>();
    reg.on_update<Gng2D::TransformLayer>()
        .disconnect<&RelationSystem::setLayerFromTransformLayer>();
    reg.on_update<Gng2D::Layer>().disconnect<&RelationSystem::updateChildrenLayer>();

    reg.on_construct<TransformPosition>()
        .disconnect<&RelationSystem::setPositionFromTransformPosition>();
    reg.on_update<TransformPosition>()
        .disconnect<&RelationSystem::setPositionFromTransformPosition>();
    reg.on_update<Gng2D::Position>().disconnect<&RelationSystem::updateChildrenPosition>();
...
void RelationSystem::updateChildrenLayer(entt::registry& reg, entt::entity parent)
{
    ...
    for (const auto child: children->list)
    {
        ...
        reg.emplace_or_replace<Layer>(
            child, static_cast<int16_t>(parentLayer.main + childTransformLayer->main),
